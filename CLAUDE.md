...
> **v3.35.4** (논문 PoC 위치 명시 + 납품 전 검증 게이트 논지): 사용자 지적 "현재 납품용 아니라 PoC 단계이니 검증해야 할 포인트를 논지로 작성". `docs/report/speclinker-paper.md`에 **§5.3 'PoC와 납품 전 검증 게이트'** 신설 — 현 산출물=*작동 가능성* 입증(PoC)이고 *효과성*은 미검증임을 명시, **반증가능한 게이트 G1~G6**(효과/비용/대안우위/충실도/가독성/규모)로 정식화. G1(그라운딩이 AIDD 결과를 실제 개선하나)이 핵심·선행, '측정 > 기능확장' 우선순위, 최소검증경로(G2 토큰계측 즉시·G1 소규모 A/B·G3 단발비교) 제안. 초록·결론에 PoC 위치 반영. HTML 덱에 PoC 게이트 슬라이드 추가. 문서 전용.
> **v3.35.3** (보고자료·논문 현행화 + CDP/RAG/AIDD-JIT 보강): `docs/report/speclinker-paper.md`·`speclinker-overview.html`를 현행 시스템(v3.35.x)에 맞춤. ①**JIT vs RAG 비교 신설**(논문 §2.1·C8 + HTML 슬라이드) — 구조그래프+소스앵커의 결정론·현행성·코드값복원 대 임베딩검색, 상보성. ②**CDP/Playwright 도입 근거 신설**(§3.6b + HTML 슬라이드) — 화면=런타임 산물이라 실DOM 필요(스택중립·권한보존·단일탭 안정화·SpecLens 이중용도). ③**AIDD 루프 JIT 활용 신설**(§3.7) — FUNC→func_context_bundle→story 번들→dev-agent 소스앵커 판독. ④RECON 페이즈 분리·inf_targets census·라우트매칭·SpecLens 반영. 문서 전용(플러그인 동작 무변경).
> **v3.35.2** (`/sl-recon-inf` 확인 게이트 + cp949 + 도메인 출처 명시): 사용자 지적 "recon-inf가 도메인을 2개로 판단하고 선택지 없이 막 생성한다 — 이전 동작과 다르다". ①**확인 게이트 누락** 복구: INF 페이즈 분리(v3.35.0) 때 생성 직전 사용자 확인이 사라졌음(구 /sl-recon STEP 3 ✋는 도메인 게이트로만 남음) → STEP 4-1(census) 직후 **✋ STEP I-1 생성범위 확인 게이트** 추가(도메인별 전체/생성/미생성 표 + [전체/특정도메인/재확정/취소], POC는 자동, 확인 전 4-2/4-3 디스패치 금지). ②**cp949 버그**: STEP 4-1이 `_domain_plan.json`·`project.env`를 encoding 없이 열어 Windows에서 UnicodeDecodeError(사용자 수동 우회) → `encoding='utf-8'` 추가. ③도메인 '2개'는 recon-inf가 판단한 게 아니라 **/sl-recon이 확정한 `_domain_plan.json`을 그대로 따른 것** — 게이트에 출처·재확정법 명시. (INF 대상 리스트업=recon-inf STEP 4-1이 `_domain_plan` 도메인별 controller→api route 스캔·INF-ID 채번으로 수행; 도메인 *자체* 확정은 /sl-recon STEP 1.7~3.) 11/11 python 블록 컴파일 그린.
> **v3.35.1** (UIS 뷰어/산출물 3종 — 탭 브레드크럼·INF 링크·팝업 TBD): 사용자 nkshop UIS-PRD-001 검수서 발견. ①**브레드크럼이 탭 이동을 반영 안 함**: docsify-sl injectBreadcrumb이 화면 ID만 표시 → 탭(하위 .md) 라우트를 감지해 `… › UIS-PRD-001 › 기초정보` 탭 세그먼트 추가 + 전 세그먼트 클릭 이동(대시보드/도메인/타입/화면), openDomainTab 헬퍼. ②**§2·§4의 INF 링크 클릭 안 됨**: ddd-ui-agent가 raw API를 백틱으로 감싸 코드로 렌더돼 마크다운 링크 사망 + addCrosslinks가 `<code>` 포함 요소를 통째 스킵. → addCrosslinks에 코드 안 ID·`[ID](path)`도 링크화하는 패스 추가(뷰어, 기존 파일 즉효) + **ddd-ui-agent 백틱 금지** + **link_uis_inf가 INF 링크 감싼 백틱 제거**(재실행 시 기존 산출물도 복구). ③**§6 팝업 '연결 INF' 전부 [TBD]** — 원인 2겹: (a)§6에 'URL/route'+'연결 INF' 두 열인데 link_uis_inf는 raw 경로를 그 자리서 링크 치환할 뿐 별도 [TBD] 열은 못 채움(죽은 열), (b)팝업 대부분은 `kind:form`(화면 열기)이라 INF가 애초에 없음(데이터호출 `…List` `kind:api`만 INF). → §6를 **단일 `연결 API/화면(raw→INF)` 열**로 재설계, form 팝업은 `화면(INF 없음)`, `[TBD]` 금지. 기존 UIS는 재생성 또는 link_uis_inf 재실행 시 반영. node --check 그린.
