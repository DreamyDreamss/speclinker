...
3. **출력은 SOP급이다.** [TBD] 위젯표가 아니라 **신규입사자가 읽고 "이 화면으로 뭘 어떻게 하는지" 이해하는 문서**를 쓴다. 핵심은 「주요 작업 시나리오」 섹션. 동시에 기계 인덱스(api_hints·anchors)도 유지(이중 레이어).
...
> **탭=섹션 vs 별도 UIS 판정 규칙:** 탭이 **독립 라우트 또는 독립 저장 엔드포인트**를 가지면 별도 화면 → 별도 UIS. 그렇지 않고 **한 라우트·한 저장(예: saveXxxInterface가 전 탭 데이터 집계)·공유 컨텍스트**면 **한 화면, 탭은 4번 섹션**. (소스의 저장 엔드포인트가 전 탭 파라미터를 모으는지로 판정.)
...
| **작업 시나리오** | 위 요소 종합 → "이 화면으로 X 하려면 (1)(2)(3)…" 흐름 (SOP 핵심) |
...
> 원 안 번호 = 「4. 위젯·액션」 표의 번호와 1:1. (멀티탭이면 4.{N}에서 `![{탭명}](tabs/tab{N}_{탭명}_annotated.png)` 참조)
...
> **이미지만 넣지 말 것. 탭마다 [이미지 + 마커 번호별 설명표]가 1세트.** 마커번호 = 이미지 원번호 = 표 번호 (3중 일치), 이미지의 모든 마커는 표에 설명 필수.

**(A) 적은 탭(≤3) → 본 spec.md에 인라인** `### 4.{N} {탭명} 탭` + 이미지 + 표.

**(B) 다탭(≥4) → 허브+분리 파일** (한 문서에 다 넣으면 비대해 가독성 붕괴 — pr201 8탭 사례):
- **spec.md(허브)**: 공통영역 표 + `### 4.9 탭 목차` (탭명·역할·저장파라미터·**상세파일 링크** 표). 탭 상세는 인라인하지 않는다.
- **`tabs/tab{N}_{탭명}.md`(탭별 분리)**: `# {탭명} 탭` + 이미지 + 마커 설명표.
  ```markdown
...
> SpecLens는 허브에서 탭 파일로 링크 네비게이션(상대경로), 탭 파일의 이미지는 문서 디렉토리 기준 렌더.
...
4. **4 표 작성 = 전수 설명**: `widgets.json`의 **모든 번호**를 4(또는 탭 파일) 표에 `번호|위젯|동작|API`로 적는다. 라벨이 DOM에서 부정확하면(예: 검색아이콘) **소스를 읽어 의미를 보정**(이게 AI의 일). **단 빠뜨리지 말 것** — widgets.json의 번호 개수 = 표 행 수 = 이미지 마커 수(3중 일치).
...
[ ] (멀티탭) 각 탭 4.{N}에 **이미지 + 마커 설명표가 둘 다** 있는가? **이미지에 찍힌 모든 마커(번호)가 표에 설명**되는가? (이미지만 덜렁 넣기 금지 — 마커마다 위젯·동작 설명)
[ ] (인터랙티브) 마커 선택은 `select_tab_widgets.py`(결정론 전수)로 했는가? **AI가 subset을 고르지 않았는가**(누락 원인)?
[ ] **누락 검사**: widgets.json의 번호 개수 = 4(탭)표 행 수 = 이미지 마커 수가 **정확히 일치**하는가? (AI가 표에서 일부를 빠뜨리면 불일치)
[ ] 0·4번 섹션 마커 번호가 캡처 원 번호와 1:1 일치하는가?
```
