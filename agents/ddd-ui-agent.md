...
화면(또는 탭)별로:
1. §4에서 문서화한 위젯의 `bbox`를 dom_snapshot(`dom_snapshot[_tab{N}].json`)에서 id/name으로 찾아 매핑.
...
1. frontmatter `api_hints`에 관측한 **raw 경로**(METHOD path)를 적는다. `link_uis_inf.py`가 INF(method,path) 인덱스와 경로조인해 §4·§6의 INF-ID를 채우고 INF의 `screens:`에 역기록한다.
2. INF가 아직 없는 경로는 `_tmp/{screenId}_inf_required.json`에 기록(ddd-api-agent 입력).
