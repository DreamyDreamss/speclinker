...
> **멀티탭이면 탭마다 아래 구조를 반복한다 — 이미지만 넣지 말 것. 이미지 + 그 탭 마커 설명표가 1세트.**

### §4.{N} {탭명} 탭
![{탭명}](tabs/tab{N}_{탭명}_annotated.png)

| № | 위젯 | 동작 | 연결 API(raw) | 결과 |
|---|------|------|--------------|------|
| ① | `#btnUntGrpInsert` 행추가 | 단품 행 추가 | — | 그리드 행 추가 |
| ② | `#btnPrdRegAtCre` 자동생성 | 옵션 조합 자동생성 | [TBD] (팝업) | 단품 일괄생성 |

> 마커 번호 = 이미지 원 안 번호 = 이 표의 № (3중 일치). **이미지에 찍힌 모든 마커는 표에 설명이 있어야 한다**(덜렁 이미지만 금지).
