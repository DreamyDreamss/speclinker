...
REQ-F-XXX
  ...
  ├─► SRS-F-XXX   (기능 명세)
  │
  ├─► UIS-F-XXX  (화면 설계)    ← REQ traces_to
  │       │ calls
  │       └─► INF-XXX          ← UIS calls (메소드 단위)
  │                 │ reads_from
  │                 └─► SCH-XXX  (DB 테이블)
  │
  ├─► INF-XXX    (인터페이스)    ← REQ traces_to (직접 연결도 가능)
  │
  └─► TC-F-XXX   (테스트케이스)  ← REQ traces_to

REQ-NF-XXX → SRS-NF-XXX → TC-NF-XXX
REQ-C-XXX  (변경 요구사항, 독립 트리)
```
...
| ----- | -------------------------------------- | ----------------------------- | -------------------------------------------- |
| `req` | `REQ-F-NNN`, `REQ-NF-NNN`, `REQ-C-NNN` | `docs/01_요구사항정의서/RD_v1.0.md`       | `| REQ-F-001 | 이름 |`                         |
| `srs` | `SRS-F-NNN`, `SRS-NF-NNN`              | `docs/03_기능명세서/SRS_v1.0.md`     | `| SRS-F-001 | 이름 | REQ-F-001 |`             |
| `uis` | `UIS-F-NNN`                            | `docs/05_설계서/UI_Spec_v1.0.md` | `| UIS-F-001 | 화면명 | REQ-F-001 |`            |
| `inf` | `INF-NNN`                              | `docs/05_설계서/API_Design.md`   | `| INF-001 | POST /path — 기능명 | REQ-F-001 |` |
| `sch` | `SCH-NNN`                              | `docs/05_설계서/DB_Schema.md`    | `| SCH-001 | 테이블명 | INF-001 |`               |
| `tc`  | `TC-F-NNN`, `TC-NF-NNN`                | `docs/07_테스트케이스/TC_v1.0.md`       | `| TC-F-001 | 이름 | REQ-F-001 |`              |
...
```
docs/
  00_input/          인터뷰·회의록 (sl-genesis 입력)
  01_RD/             REQ SoT
    RD_v1.0.md       ← 색인 (RD_v1.0.md 우선)
    req/             REQ-F-XXX.md 개별 파일 (선택)
...
```
| REQ-ID | 요구사항명 | SRS-ID | UIS-ID | INF-ID | SCH-ID | TC-ID | 코드 | 상태 |
```
...
4. 코드 파일은 linked_req 주석 또는 RTM 코드 열에서 확인
...
| ------------ | ----------------------------------------------- |
| 전체 체인 확인     | RTM 탭 → 표 확인                                    |
| 특정 REQ 영향 범위 | REQ 노드 클릭 → 연결 노드 확인                            |
...
> 이 파일은 `sl-genesis`, `sl-dev`, `sl-test` 실행 전 자동으로 참조된다.
