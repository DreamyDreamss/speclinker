...
| 2026-05-27 | ddd-ui-agent 재설계 + UIS↔INF 2단계 파이프라인 ★★★. (1) ddd-ui-agent.md Phase 3 JSP/jwork 블록 전면 재설계: JSP HTML에서 버튼 ID 수집 + `<script src>` 포함 JS 파일 전부 읽기 + `$(#btnId).on("click")` + `$.ajax({url:...})` 패턴 추출 → interaction_map 완성 (기존에 JSP만 읽어 §5 불가였던 근본 결함 해소). (2) 프레임워크별 URL 추출 가능 여부 표로 정리 (JSP form action/포함 JS = 추출 가능, Django URL 헬퍼 = 추출 불가). (3) Phase 7.5 신설: `_tmp/{화면ID}_inf_required.json` 출력 — INF 없는 URL 목록 저장. (4) `scripts/link_uis_inf.py` 신규: INF 생성 완료 후 UIS spec.md §5의 URL을 INF 링크로 LLM 없이 스크립트 패치. (5) Self-Critique에 JSP JS 파일 읽기 필수 항목 추가. v2.6.0 버전 업. | Claude |
