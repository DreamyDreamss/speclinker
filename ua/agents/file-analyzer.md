...
**Monorepos and web apps:** If the repository has a dedicated frontend or service package (e.g. `apps/web`, `packages/ui`, `*-web/src`), ensure batches include those paths so `knowledge-graph.json` contains file nodes under those trees — not only the repository root. ASIS RTM and `linked-req-cache.json` rely on the same relative paths appearing in the graph.
