...
const SPEC_BADGE: Record<string, { bg: string; color: string }> = {
  req:  { bg: "rgba(167,139,250,0.12)", color: "var(--color-spec-req)" },
  srs:  { bg: "rgba(147,197,253,0.12)", color: "var(--color-spec-srs)" },
  uis:  { bg: "rgba(251,146,60,0.12)",  color: "var(--color-spec-uis)" },
  inf:  { bg: "rgba(45,212,191,0.12)",  color: "var(--color-spec-inf)" },
  sch:  { bg: "rgba(74,222,128,0.12)",  color: "var(--color-spec-sch)" },
  tc:   { bg: "rgba(148,163,184,0.12)", color: "var(--color-spec-tc)"  },
  func: { bg: "rgba(251,191,36,0.12)",  color: "var(--color-spec-func)" },
};

function SpecLinks({ nodeId }: { nodeId: string }) {
  const siGraph = useDashboardStore((s) => s.siGraph);
  const setSelectedReqId = useDashboardStore((s) => s.setSelectedReqId);

  if (!siGraph) return null;

  const satisfiesEdges = siGraph.edges.filter(
    (e) => e.type === "satisfies" && e.source === nodeId
  );
  if (satisfiesEdges.length === 0) return null;

  const reqNodes = satisfiesEdges
    .map((e) => siGraph.nodes.find((n) => n.id === e.target))
    .filter((n): n is GraphNode => n !== undefined && n.type === "req");
  if (reqNodes.length === 0) return null;

  const specChain = reqNodes.map((req) => {
    const reqId = req.siMeta?.reqId ?? req.name;

    const linked = (type: string, sourceId: string, edgeType: string) =>
      siGraph.edges
        .filter((e) => e.type === edgeType && e.source === sourceId)
        .map((e) => siGraph.nodes.find((n) => n.id === e.target && n.type === type))
        .filter((n): n is GraphNode => n !== undefined);

    const srsNodes = linked("srs", req.id, "traces_to");
    const uisNodes = linked("uis", req.id, "traces_to");
    const infNodes = uisNodes.flatMap((u) => linked("inf", u.id, "calls"));
    const schNodes = infNodes.flatMap((i) => linked("sch", i.id, "reads_from"));
    const tcNodes = siGraph.nodes.filter(
      (n) => n.type === "tc" && n.siMeta?.linkedReqId === reqId
    );

    return { req, reqId, srsNodes, uisNodes, infNodes, schNodes, tcNodes };
  });

  const specBadge = (type: string, id: string, label: string) => {
    const s = SPEC_BADGE[type] ?? SPEC_BADGE.tc;
    return (
      <span
        key={id}
  ...
  return (
    <div className="mt-4">
      <h3 className="text-[11px] font-semibold text-purple-400 uppercase tracking-wider mb-2">
        연결된 스펙
      </h3>
      <div className="space-y-2">
        {specChain.map(({ req, reqId, srsNodes, uisNodes, infNodes, schNodes, tcNodes }) => (
          <div key={req.id} className="bg-elevated rounded-lg p-2 border border-purple-900/30">
            <button
              type="button"
              onClick={() => setSelectedReqId(reqId)}
              className="text-[10px] font-mono px-2 py-0.5 rounded border hover:brightness-125 transition-all"
              style={{ backgroundColor: SPEC_BADGE.req.bg, color: SPEC_BADGE.req.color, borderColor: `${SPEC_BADGE.req.color}40` }}
            >
              {reqId}
            </button>
            <p className="text-[10px] text-text-muted mt-1 mb-2 line-clamp-1">{req.name}</p>
            <div className="flex flex-wrap gap-1">
              {srsNodes.map((n) => specBadge("srs", n.id, n.siMeta?.reqId ?? n.name))}
              {uisNodes.map((n) => specBadge("uis", n.id, n.siMeta?.reqId ?? n.name))}
              {infNodes.map((n) => specBadge("inf", n.id, n.siMeta?.reqId ?? n.name))}
              {schNodes.map((n) => specBadge("sch", n.id, n.siMeta?.reqId ?? n.name))}
              {tcNodes.map((n) => specBadge("tc", n.id, n.siMeta?.reqId ?? n.name))}
            </div>
          </div>
        ))}
      </div>
...
// ── DomainSpecLinks: file-based spec links (no si-graph required) ─────────────

function DomainSpecLinks({ node }: { node: GraphNode }) {
  ...
  // If si-graph is already handling this, skip to avoid duplication
  if (siGraph) return null;
  if (!infList.length && !uisList.length) return null;

  // Determine domain from node's filePath (keyword match)
  const fp = (node.filePath ?? node.name).toLowerCase().replace(/\\/g, "/");
  const allDomains = Array.from(new Set([
    ...infList.map(i => i.domain),
    ...uisList.map(u => u.domain),
  ]));
  const matchedDomain = allDomains.find(d => {
    const dl = d.toLowerCase();
    return fp.includes(dl) || dl.includes(fp.split("/").pop()?.replace(/\.[^.]+$/, "") ?? "___");
  });

  if (!matchedDomain) return null;

  const domainInfs = infList.filter(i => i.domain === matchedDomain);
  const domainUis  = uisList.filter(u => u.domain === matchedDomain);
  const total = domainInfs.length + domainUis.length;
  if (total === 0) return null;

  const handleSelect = (path: string) => {
    if (selectedPath === path) { setSelectedPath(null); setContent(""); return; }
    setSelectedPath(path);
    fetch(`/spec-file?path=${encodeURIComponent(path)}`)
      .then(r => r.ok ? r.text() : "")
      .then(setContent)
  ...
  return (
    <div className="mt-4">
      ...
      >
        <h3 className="text-[11px] font-semibold text-teal-400 uppercase tracking-wider">
          관련 스펙 문서
        </h3>
        <span className="text-[10px] text-text-muted font-mono">{matchedDomain}</span>
        <span className="text-[10px] text-text-muted ml-auto">{open ? "▾" : "▸"} {total}개</span>
      </button>
      {open && (
          ...
          {domainInfs.map(item => (
            <div key={item.infId}>
              <button
                ...
                className={`w-full text-left flex items-center gap-2 px-2 py-1.5 rounded-lg border transition-colors text-[11px] ${
                  selectedPath === item.path
                    ? "bg-teal-900/30 border-teal-700/40"
                    : "bg-elevated border-border-subtle hover:border-teal-700/30 hover:bg-teal-900/10"
                }`}
              >
                <span className="font-mono text-[9px] px-1.5 py-0.5 rounded border text-teal-400 bg-teal-900/40 border-teal-700/40 shrink-0">
                  INF
                </span>
                <span className="font-mono text-text-secondary">{item.infId}</span>
                <span className="text-text-muted ml-auto text-[10px] shrink-0">
                  {selectedPath === item.path ? "▾" : "▸"}
                </span>
              </button>
              {selectedPath === item.path && content && (
                <div className="mt-1 mx-1 rounded-lg bg-surface border border-border-subtle overflow-hidden">
                  <SpecMarkdown content={content} maxHeight="220px" className="p-2.5" />
                </div>
              )}
            </div>
          ))}
              ...
              <button
                ...
                className={`w-full text-left flex items-center gap-2 px-2 py-1.5 rounded-lg border transition-colors text-[11px] ${
                  selectedPath === item.path
                    ...
                    : "bg-elevated border-border-subtle hover:border-orange-700/30 hover:bg-orange-900/10"
                }`}
              >
                <span className="font-mono text-[9px] px-1.5 py-0.5 rounded border text-orange-400 bg-orange-900/40 border-orange-700/40 shrink-0">
                  UIS
                </span>
                <span className="font-mono text-text-secondary">{item.uisId}</span>
                {item.route && <span className="text-text-muted text-[10px] truncate">{item.route}</span>}
                <span className="text-text-muted ml-auto text-[10px] shrink-0">
                  {selectedPath === item.path ? "▾" : "▸"}
                </span>
              </button>
...
// ── FuncLinks: FUNC-ID badges from linkedFuncMap ─────────────────────────────

function FuncLinks({ node }: { node: GraphNode }) {
  const funcMap = useDashboardStore((s) => s.funcMap);
  const linkedFuncMap = useDashboardStore((s) => s.linkedFuncMap);
  const [open, setOpen] = useState(false);

  if (!linkedFuncMap || !node.filePath) return null;

  const fp = node.filePath.replace(/\\/g, "/");
  const matchedFuncIds: string[] = [];
  for (const [funcId, entries] of Object.entries(linkedFuncMap)) {
    if (entries.some((e) => {
      const ef = e.file.replace(/\\/g, "/");
      return fp.endsWith(ef) || ef.endsWith(fp);
    })) {
      matchedFuncIds.push(funcId);
    }
  }

  if (matchedFuncIds.length === 0) return null;
  ...
  return (
    <div className="mt-4">
      ...
      >
        <h3 className="text-[11px] font-semibold text-amber-400 uppercase tracking-wider">
          연결된 FUNC
        </h3>
        <span className="text-[10px] text-text-muted ml-auto">{open ? "▾" : "▸"} {matchedFuncIds.length}개</span>
      </button>
      {open && (
        <div className="space-y-1.5">
          {matchedFuncIds.map((funcId) => {
            const func = funcMap?.find((f) => f.id === funcId);
            const refs = linkedFuncMap[funcId] ?? [];
            return (
              <div key={funcId} className="flex items-start gap-2 px-2 py-1.5 rounded-lg bg-elevated border border-amber-700/20">
                <span className="font-mono text-[9px] px-1.5 py-0.5 rounded border bg-amber-900/40 text-amber-400 border-amber-700/40 shrink-0 mt-0.5">
                  {funcId}
                </span>
                <div className="flex-1 min-w-0">
                  {func?.description && (
                    <div className="text-[10px] text-text-secondary truncate">{func.description}</div>
                  )}
                  {refs.slice(0, 2).map((r, i) => (
                    <div key={i} className="text-[9px] font-mono text-text-muted">
...
const typeBadgeColors: Record<NodeType, string> = {
  ...
  req: "text-node-document border border-node-document/30 bg-node-document/10",
  srs: "text-node-document border border-node-document/30 bg-node-document/10",
  tc: "text-node-function border border-node-function/30 bg-node-function/10",
  uis: "text-node-endpoint border border-node-endpoint/30 bg-node-endpoint/10",
  inf: "text-node-service border border-node-service/30 bg-node-service/10",
  sch: "text-node-table border border-node-table/30 bg-node-table/10",
  // speclinker UA agent types
  entrypoint: "text-node-service border border-node-service/30 bg-node-service/10",
  router: "text-node-endpoint border border-node-endpoint/30 bg-node-endpoint/10",
  utility: "text-node-module border border-node-module/30 bg-node-module/10",
  scheduler: "text-node-pipeline border border-node-pipeline/30 bg-node-pipeline/10",
  batch_job: "text-node-pipeline border border-node-pipeline/30 bg-node-pipeline/10",
  crawler: "text-node-pipeline border border-node-pipeline/30 bg-node-pipeline/10",
  page: "text-node-document border border-node-document/30 bg-node-document/10",
  query: "text-node-resource border border-node-resource/30 bg-node-resource/10",
};
...
const EDGE_LABELS: Record<EdgeType, { forward: string; backward: string }> = {
  ...
  satisfies: { forward: "satisfies", backward: "satisfied by" },
  traces_to: { forward: "traces to", backward: "traced by" },
};
...
export default function NodeInfo() {
  ...
  return (
    <div className="h-full w-full overflow-auto p-5 animate-fade-slide-in">
      ...
      {/* Linked specs from si-graph */}
      <SpecLinks nodeId={node.id} />

      {/* Domain-based spec file links (file-based fallback) */}
      <DomainSpecLinks node={node} />

      {/* FUNC-ID badges from linkedFuncMap */}
      <FuncLinks node={node} />
    </div>
  );
}
