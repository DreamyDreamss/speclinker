...
const typeColors: Record<NodeType, string> = {
  ...
  req: "var(--color-node-document)",
  srs: "var(--color-node-document)",
  tc: "var(--color-node-function)",
  uis: "var(--color-node-endpoint)",
  inf: "var(--color-node-service)",
  sch: "var(--color-node-table)",
  entrypoint: "var(--color-node-service)",
  router: "var(--color-node-endpoint)",
  utility: "var(--color-node-module)",
  scheduler: "var(--color-node-pipeline)",
  batch_job: "var(--color-node-pipeline)",
  crawler: "var(--color-node-pipeline)",
  page: "var(--color-node-document)",
  query: "var(--color-node-resource)",
};
...
const typeTextColors: Record<NodeType, string> = {
  ...
  req: "text-node-document",
  srs: "text-node-document",
  tc: "text-node-function",
  uis: "text-node-endpoint",
  inf: "text-node-service",
  sch: "text-node-table",
  entrypoint: "text-node-service",
  router: "text-node-endpoint",
  utility: "text-node-module",
  scheduler: "text-node-pipeline",
  batch_job: "text-node-pipeline",
  crawler: "text-node-pipeline",
  page: "text-node-document",
  query: "text-node-resource",
};
  ...
  return (
    ...
    >
      ...
      <div className="pl-4 pr-3 py-2">
        ...
        {/* REQ-ID 배지 (req/srs/tc 노드용) */}
        {(data as any).siMeta?.reqId && (
          <span className="text-xs opacity-70 block truncate font-mono">
            {(data as any).siMeta.reqId}
          </span>
        )}
      </div>
