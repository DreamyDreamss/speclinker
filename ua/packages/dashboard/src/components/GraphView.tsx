...
import DirClusterNode from "./DirClusterNode";
import type { DirClusterFlowNode } from "./DirClusterNode";
...
import {
  ...
  PORTAL_NODE_WIDTH,
  PORTAL_NODE_HEIGHT,
  DIR_CLUSTER_NODE_WIDTH,
  DIR_CLUSTER_NODE_HEIGHT,
} from "../utils/layout";
import {
  aggregateLayerEdges,
  computePortals,
...
const nodeTypes = {
  ...
  "dir-cluster": DirClusterNode,
  portal: PortalNode,
};

const DIR_CLUSTER_THRESHOLD = 20;

function getDirPrefix(filePath: string | undefined): string {
  if (!filePath) return "(root)";
  const parts = filePath.split("/");
  if (parts.length <= 1) return "(root)";
  const first = parts[0];
  if (
    (first === "src" || first === "lib" || first === "app" ||
     first === "packages" || first === "apps" || first === "source") &&
    parts.length > 2
  ) {
...
/** Returns the sub-group key for a node within a parent dir prefix.
 *  Returns null for direct files (no sub-directory beyond the prefix). */
function computeGroupKey(filePath: string | undefined, parentPrefix: string | null): string | null {
  if (!filePath) return null;
  if (parentPrefix === null) {
    const p = getDirPrefix(filePath);
    return p === "(root)" ? null : p;
  }
  const pw = parentPrefix + "/";
  if (!filePath.startsWith(pw)) return null;
  const rest = filePath.slice(pw.length);
  if (!rest.includes("/")) return null; // direct file in this dir, no sub-group
  return pw + rest.split("/")[0];
}

import type { NodeCategory, Complexity } from "../store";
...
const NODE_TYPE_TO_CATEGORY: Record<NodeType, NodeCategory> = {
  ...
  req: "docs", srs: "docs", tc: "code", uis: "docs", inf: "docs", sch: "data",
  entrypoint: "infra", router: "code", utility: "code",
  scheduler: "infra", batch_job: "infra", crawler: "infra",
  page: "docs", query: "data",
} as const;
...
function useLayerDetailTopology() {
  ...
  const filterComplexities = useDashboardStore((s) => s.filters.complexities);
  ...
  return useMemo(() => {
    ...
    filteredGraphNodes = filteredGraphNodes.filter((n) =>
      filterComplexities.has(n.complexity as Complexity)
    );
    ...
    // When drilling into a specific dir prefix, filter nodes using startsWith (supports deep recursion)
    if (activeDirPrefix !== null) {
      filteredGraphNodes = filteredGraphNodes.filter(
        (n) => n.filePath !== undefined && (
          n.filePath === activeDirPrefix ||
          n.filePath.startsWith(activeDirPrefix + "/")
        )
      );
    ...
    // Unified dir/sub-dir clustering — runs at both layer-top and inside any prefix
    if (!focusNodeId && filteredGraphNodes.length > DIR_CLUSTER_THRESHOLD) {
      const dirGroups = new Map<string, typeof filteredGraphNodes>();
      for (const node of filteredGraphNodes) {
        const key = computeGroupKey(node.filePath, activeDirPrefix);
        if (key === null) continue; // skip direct files (no sub-dir)
        if (!dirGroups.has(key)) dirGroups.set(key, []);
        dirGroups.get(key)!.push(node);
      }

      if (dirGroups.size > 1) {
        const nodeToDir = new Map<string, string>();
        for (const [dir, nodes] of dirGroups) {
          for (const n of nodes) nodeToDir.set(n.id, dir);
        }
        const dirEdgeMap = new Map<string, number>();
        for (const edge of filteredGraphEdges) {
          const srcDir = nodeToDir.get(edge.source);
          const tgtDir = nodeToDir.get(edge.target);
          if (!srcDir || !tgtDir || srcDir === tgtDir) continue;
          const key = `${srcDir}|||${tgtDir}`;
          dirEdgeMap.set(key, (dirEdgeMap.get(key) ?? 0) + 1);
        }

        const dirClusterNodes: DirClusterFlowNode[] = Array.from(dirGroups.entries()).map(([prefix, nodes]) => {
          const complexCounts = { simple: 0, moderate: 0, complex: 0 };
          for (const n of nodes) {
            const c = n.complexity as keyof typeof complexCounts;
            if (c in complexCounts) complexCounts[c]++;
          }
          const aggComplexity =
            complexCounts.complex > nodes.length * 0.3 ? "complex" :
            complexCounts.moderate > nodes.length * 0.3 ? "moderate" : "simple";
          return {
            id: `dir:${prefix}`,
            type: "dir-cluster" as const,
            position: { x: 0, y: 0 },
            data: { dirPrefix: prefix, fileCount: nodes.length, aggregateComplexity: aggComplexity, onDrillIn: drillIntoDirPrefix },
          };
        });

        const dirFlowEdges: Edge[] = Array.from(dirEdgeMap.entries()).map(([key, count], i) => {
          const sep = key.indexOf("|||");
          const src = key.slice(0, sep);
          const tgt = key.slice(sep + 3);
          return {
            id: `de-${i}`,
            source: `dir:${src}`,
            target: `dir:${tgt}`,
            label: `${count}`,
            style: { stroke: "rgba(212,165,116,0.4)", strokeWidth: Math.min(1 + Math.log2(count + 1), 4) },
            labelStyle: { fill: "#a39787", fontSize: 11 },
          };
        });

        const dirDims = new Map<string, { width: number; height: number }>();
        for (const n of dirClusterNodes) {
          dirDims.set(n.id, { width: DIR_CLUSTER_NODE_WIDTH, height: DIR_CLUSTER_NODE_HEIGHT });
        }
        const dirLaid = applyDagreLayout(dirClusterNodes as unknown as Node[], dirFlowEdges, "TB", dirDims);
        return { nodes: dirLaid.nodes, edges: dirLaid.edges, portalNodes: [] as PortalFlowNode[], portalEdges: [] as Edge[], filteredEdges: filteredGraphEdges };
      }
    }
...
function useLayerDetailGraph() {
  ...
  const nodes = useMemo(() => {
    ...
    return topo.nodes.map((node) => {
      // Skip portal and dir-cluster nodes — they have no CustomNodeData
      if (node.type === "portal" || node.type === "dir-cluster") return node;
...
function GraphViewInner() {
  ...
  const onNodeClick = useCallback(
    (_: React.MouseEvent, node: { id: string }) => {
      if (navigationLevel === "overview") {
        ...
        drillIntoDirPrefix(node.id.slice(4));
      } else {
        selectNode(node.id);
      }
    },
    [navigationLevel, drillIntoLayer, drillIntoDirPrefix, selectNode],
  );
  ...
  return (
    <div className="h-full w-full relative">
      <Breadcrumb />
      {focusNodeId && navigationLevel === "layer-detail" && activeDirPrefix !== null && (
