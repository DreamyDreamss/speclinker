// Node types (35 total: 5 code + 8 non-code + 3 domain + 5 knowledge + 6 SI + 8 speclinker UA)
// SI spec chain: req → srs → uis → inf(인터페이스) → sch (← TC)
// speclinker UA: entrypoint router utility scheduler batch_job crawler page query
...
// Optional SI metadata for req/srs/tc/uis/api/sch nodes
export interface SIMeta {
  reqId: string;
  status?: "⬜ 미착수" | "🔄 진행중" | "🧪 테스트중" | "✅ 완료" | "❌ 제외";
  linkedReqId?: string;  // 상위 체인 ID (e.g. uis→api 경우 uisId, api→sch 경우 apiId)
}

// GraphNode with 25 types: 5 code + 8 non-code + 3 domain + 5 knowledge + 4 SI
