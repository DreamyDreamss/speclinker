...
describe("schema validation", () => {
  ...
  it("accepts string project after sanitize pipeline", () => {
    const graph = structuredClone(validGraph);
    (graph as any).project = "string-only-project";
    const result = validateGraph(graph);
    expect(result.success).toBe(true);
    expect(result.data!.project.name).toBe("string-only-project");
  });

  it('normalizes "directory" node type to "module"', () => {
    const graph = structuredClone(validGraph);
    (graph.nodes[0] as any).type = "directory";

    const result = validateGraph(graph);
    expect(result.success).toBe(true);
    expect(result.data!.nodes[0].type).toBe("module");
  });
...
describe("sanitizeGraph", () => {
  ...
  it("coerces string project to full metadata object", () => {
    const graph = structuredClone(validGraph);
    (graph as any).project = "my-app";
    const result = sanitizeGraph(graph as any);
    expect((result as any).project).toMatchObject({
      name: "my-app",
      languages: [],
      frameworks: [],
    });
    expect(typeof (result as any).project.analyzedAt).toBe("string");
    expect(typeof (result as any).project.gitCommitHash).toBe("string");
  });

  it("maps edge from/to to source/target", () => {
    const graph = structuredClone(validGraph);
    (graph.nodes as any[]).push({
      id: "node-2",
      type: "file",
      name: "util.ts",
...
describe("permissive validation", () => {
  ...
  it("defaults project when top-level project key is missing", () => {
    const graph = structuredClone(validGraph);
    delete (graph as any).project;

    const result = validateGraph(graph);
    expect(result.success).toBe(true);
    expect(result.data!.project.name).toBe("codebase");
  });

  it("returns fatal when project is not an object or string", () => {
    const graph = structuredClone(validGraph);
    (graph as any).project = ["not", "valid"];
