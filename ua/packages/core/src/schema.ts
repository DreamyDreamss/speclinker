...
export const EdgeTypeSchema = z.enum([
  ...
  "satisfies", "traces_to", // SI-Tracing
]);
...
function defaultProjectMeta(name: string): Record<string, unknown> {
  return {
    name,
    languages: [],
    frameworks: [],
...
/** Coerce LLM / legacy graph shapes into schema-friendly top-level fields. */
export function sanitizeGraph(data: Record<string, unknown>): Record<string, unknown> {
  const result = { ...data };

  // Project: string or partial object → ProjectMeta-compatible shape (avoids spread-on-string bugs downstream)
  const proj = data.project;
  if (proj === undefined || proj === null) {
    result.project = defaultProjectMeta("codebase");
  } else if (typeof proj === "string") {
    result.project = defaultProjectMeta(proj.trim() || "codebase");
  } else if (typeof proj === "object" && !Array.isArray(proj)) {
    const p = proj as Record<string, unknown>;
    const name = typeof p.name === "string" && p.name.trim() ? p.name : "codebase";
    const d = defaultProjectMeta(name);
    result.project = {
      ...d,
      ...p,
      name,
      languages: Array.isArray(p.languages) ? p.languages : d.languages,
      frameworks: Array.isArray(p.frameworks) ? p.frameworks : d.frameworks,
      description: typeof p.description === "string" ? p.description : d.description,
      analyzedAt: typeof p.analyzedAt === "string" ? p.analyzedAt : d.analyzedAt,
      gitCommitHash: typeof p.gitCommitHash === "string" ? p.gitCommitHash : d.gitCommitHash,
    };
  } else if (proj !== undefined && proj !== null) {
    // number, array, boolean — cannot coerce; strip so validateGraph surfaces a clear fatal
  ...
  if (Array.isArray(data.edges)) {
    result.edges = (data.edges as Record<string, unknown>[]).map((edge) => {
      ...
      const e = { ...edge };
      // Legacy / alternate field names → source/target (Cytoscape-style from/to, etc.)
      if (
        typeof e.from === "string" &&
        typeof e.to === "string" &&
        (e.source === undefined || e.target === undefined)
      ) {
        if (e.source === undefined) e.source = e.from;
        if (e.target === undefined) e.target = e.to;
        delete e.from;
        delete e.to;
      }
...
export const GraphNodeSchema = z.object({
  ...
  type: z.enum([
    ...
    "req", "srs", "tc", "uis", "inf", "sch",
    // speclinker UA agent types
    "entrypoint", "router", "utility", "scheduler", "batch_job", "crawler", "page", "query",
  ]),
  name: z.string(),
