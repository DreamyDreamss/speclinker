...
브리프의 소스앵커 `file:line`을 **Read 도구로 직접 읽어** 현재 비즈니스 규칙·구현을 정밀 확인한다(요약 스펙의 손실 회피). 관련도 상위 INF부터 본다.
**ripple(연결경로)의 시드 외 사용처는 회귀 영향분석(Step 6)에 반드시 반영**한다.

- **⚠️ 현행성 경고** 섹션의 INF는 소스가 스펙보다 최신(stale) → 그래프/스펙 대신 **소스를 1차 진실**로 보고, 필요시 해당 도메인 재RECON 권고.
- **⚠️ 광역 공통자원**(JT_CODE 등)은 영향이 전역적 → 개별 나열 대신 **신중 검토 대상**으로 사용자에게 명시.
...
mcp-atlassian 호출:
  ...
  args:
    ...
    body: |
      ...
      RTM: SR-{ID} 변경 행 추가, 관련 항목 상태 → 🔁 변경중
