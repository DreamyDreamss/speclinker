...
- **대시보드**: 도메인별 INF/UIS 수 + 스펙완성도/개발완료율. 헤더 클릭 정렬, **연결 갭 배지**(화면-API/API-테이블 미연결 수), 인덱스 stale 경고.
- **글로벌 검색**: 사이드바 상단 검색창에 INF·화면·테이블·경로 입력 → 즉시 점프.
- **도메인 탭**: 사이드바에서 도메인 선택 → INF/UIS/SCH(/BAT) 탭 전환. BAT는 산출물 있을 때만 표시.
- **상세 + 연결관계 패널**: INF/UIS/SCH 카드 클릭 → Docsify 렌더링 + **브레드크럼** + 우측 **🔗 연결관계 패널**(UIS=호출 API/관련 테이블, INF=관련 테이블/사용 화면, SCH=참조 API, 공통 linked FUNC, 클릭 시 이동).
