...
### STEP 6-1: 선택 범위별 BFS 탐색 + 캡처

`bfs_scope_selection.json`의 각 스코프에 대해 순차로 실행한다.  
각 실행은 **실제 메뉴를 클릭**하여 화면을 탐색하고, `--capture` 시 스크린샷을 찍는다.

> PREVIEW_BASE_URL 미설정 → `--capture` 없이 탐색만 실행 (화면 목록만 수집)
...
scopes = sel.get('scopes', [''])   # 빈 문자열 = 전체

do_capture = bool(base_url)
all_flat = []
...
merged = {
    ...
    'stats': {
        'screens': len(all_flat),
        'captured': len([s for s in all_flat if s.get('captureStatus') == 'done']),
    },
    'flat': all_flat,
}
json.dump(merged, open('_tmp/screen_hierarchy.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=2)
print(f'\\n[병합 완료] 총 화면 {len(all_flat)}개 → _tmp/screen_hierarchy.json')
"
```

---

### STEP 6-2: 도메인 배정 + 화면 인벤토리 생성
...
# 도메인 키워드 맵 생성 (name, description, rootPaths basename 기반)
domain_kw = {}
...
# UIS ID 범위 맵
uis_ranges = {d['name']: d.get('uis', {'start': 1, 'end': 999}) for d in plan['domains']}
...
for screen in flat:
    ...
    # captureFile: _tmp/captures/{screenId}/preview.png → 나중에 이동
    ...
    inventory.append({
        ...
        'entryFile':      '',   # STEP 6-3.5에서 source analyze
        'componentFiles': [],
    })
...
if unassigned:
    ...
    for s in inventory:
        if s['domain'] == 'unassigned':
            print(f'  {s[\"screenId\"]:30} route={s[\"route\"]}  menuPath={\">\".join(s[\"menuPath\"])}')
"
```

도메인 배정이 잘못된 항목이 있으면 `_tmp/screen_inventory.json`을 직접 수정하거나 사용자에게 수정 안내한다.

---

### ✋ STEP 6-2.5: 도메인 선택 체크포인트 (필수)
...
# ← 사용자 응답에 따라 채운다
selected_domains = []   # 빈 리스트 = 전체

inv = json.load(open('_tmp/screen_inventory.json', encoding='utf-8'))
if selected_domains:
    filtered = [s for s in inv if s.get('domain') in selected_domains]
    print(f'선택: {selected_domains} → {len(filtered)}개 화면 처리')
else:
    filtered = inv
    print(f'전체 선택 → {len(filtered)}개 화면 처리')
...
### STEP 6-3: [A] generate_uis_spec.py — 시각 spec.md 초안

캡처(`widgets.json` 또는 `preview_*.png`)가 있는 화면에만 실행한다.  
없는 화면은 이 단계를 스킵하고 STEP 6-4(ddd-ui-agent)가 전체 spec.md를 생성한다.
...
### STEP 6-4: [B] ddd-ui-agent — 소스 분석 + §5 작성 (항상 실행)
