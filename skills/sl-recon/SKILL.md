...
for d in plan['domains']:
    ...
    # ID 범위 오버플로우 감지 — 파일 수 > 슬롯 수 → 충돌 경고
    if len(files) > total_slots:
        suggested_end = inf_s + len(files) - 1
        print(f'  [WARNING] {d["name"]}: 컨트롤러 {len(files)}개 > INF ID 범위 {total_slots}개 (INF-{inf_s:03d}~INF-{inf_e:03d})')
        print(f'    → _domain_plan.json의 {d["name"]}.inf.end를 {suggested_end}으로 올리세요.')
        print(f'    → 현재 상태로 진행하면 INF-ID 중복 충돌이 발생합니다.')

    # filePath → api routes 룩업 (kind=form 제외 — ddd-api-agent가 form 라우트 처리 방지)
    # norm() + lower() 이중 정규화: path separator 차이 및 대소문자 불일치로 인한 키 미스 방지
    def norm_key(p):
        return norm(p).lower()
    ...
    for i, fp in enumerate(files):
        ...
        all_items.append({
            ...
            'infStart': s,
            'infEnd': e,
            'apiRoutes': api_routes_lookup.get(norm_key(fp), []),
        })
...
`ddd-db-agent`를 도메인별로 호출한다. (동시에 3도메인씩):

```
docs/05_설계서/_domain_plan.json의 각 도메인에 대해 Agent 도구 호출:
  subagent_type: "speclinker:ddd-db-agent"
  description: "{domain.name} SCH 생성"
  prompt: |
    도메인: {domain.name}
    도메인 설명: {domain.description}
    SCH ID 범위: SCH-{domain.sch.start:03d} ~ SCH-{domain.sch.end:03d}
    ...
    완료 후:
    - docs/05_설계서/{domain.name}/DB_Schema_{domain.name}.md 생성
    - SCH-XXX 섹션은 INF tables: 와 양방향 링크 포함
```
