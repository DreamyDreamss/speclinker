...
## STEP 2.7 — 런타임 BFS 메뉴 탐색 (Phase 7.4, 선택)

> **트리거**: STEP 2.5-2에서 사용자가 **b) runtime 보강 필요** 를 선택한 경우에만 실행.  
> Chrome `--remote-debugging-port=9222` 로 앱을 로그인까지 마친 상태에서 아래를 실행.

### STEP 2.7-1: BFS 탐색 실행
...
!python3 -c "
import json, os
path = '_tmp/screen_plan_runtime.json'
if not os.path.exists(path):
    print('[ERROR] screen_plan_runtime.json 없음 — capture.js --traverse-menu 실패 확인')
else:
    data = json.load(open(path, encoding='utf-8'))
    screens = data.get('screens', [])
    disc = data.get('discovery', {})
    print(f'BFS 탐색 결과: {disc.get(\"runtime_count\", 0)}개 화면')
    for s in screens[:10]:
        print(f'  {s[\"route\"]:50} [{s[\"metadata\"].get(\"menu_l1\",\"\")}/{s[\"metadata\"].get(\"menu_l2\",\"\")}]')
    if len(screens) > 10:
        print(f'  ... 외 {len(screens)-10}개')
"
```

### STEP 2.7-2: 정적 + 런타임 병합
...
!python3 -c "
import json, os
path = '_tmp/screen_plan_merged.json'
if not os.path.exists(path):
    print('[ERROR] screen_plan_merged.json 없음')
else:
    data = json.load(open(path, encoding='utf-8'))
    disc = data.get('discovery', {})
    print(f'병합 결과: 총 {disc.get(\"merged_total\",0)}개')
    print(f'  정적: {disc.get(\"static_count\",0)}개 | 런타임 신규 추가: {disc.get(\"runtime_added\",0)}개 | 메뉴정보 보강: {disc.get(\"runtime_enriched\",0)}개')
"
```

### STEP 2.7-3: 병합 결과 confirmed.json 저장
...
> SQL 텍스트에서 추출한 도메인별 테이블·컬럼·근거 파일·INF 범위 매핑이 들어 있으며, STEP 5의 `ddd-db-agent` 1차 입력으로 사용된다.  
...
동시에 3그룹씩 Agent 도구 호출, 완료 후 다음 3그룹:
  ...
  prompt: |
    ...
    MODE: RECON
    워크스페이스: {현재 작업 디렉토리 절대경로}
    프로젝트 Profile: .speclinker/profile.yaml (있으면 framework/architecture 인식에 사용, 없으면 기본 패턴)
...
gaps가 있으면 아래 순서로 **INF 역주입 루프** 실행:

```
gaps 항목 3개씩 묶어 Agent 도구 호출:
  subagent_type: "speclinker:ddd-api-agent"
...
```bash
# INF 생성 후 spec.md 재생성 — api_hints ↔ INF 매칭 갱신
!python3 "%PLUGIN_PATH%\scripts\generate_uis_spec.py" ^
       docs/05_설계서/{domain}/UI/{화면ID} ^
       --uis-id=UIS-F-{NNN} --screen-id={화면ID} --screen-name={화면명} ^
       --route={route} --domain={domain}
...
print(f'역주입 후 gaps: {len(remaining)}건 (0건이면 완전 매칭)')
"
```
