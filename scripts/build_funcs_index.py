...
읽기:
  docs/05_설계서/{도메인}/UIS/{화면ID}/spec.md  (구버전 UI/ 하위호환)
...
def main():
    ...
    for domain in sorted(os.listdir(DOCS) if os.path.isdir(DOCS) else []):
        # 화면 디렉토리: 현행 'UIS'(v3.9~) 우선, 구버전 'UI' 하위호환
        ...
        for screen_id in sorted(os.listdir(ui_dir)):
            ...
            ref_inf_ids = sorted(set(re.findall(r'INF-[A-Z]+-\d+', body)))
            ...
            func_entry = {
                ...
                'specPath':   f'docs/05_설계서/{domain}/{ui_dirname}/{screen_id}/spec.md',
                'uisId':      uis_id,
                'route':      route,
                'api_hints':  api_hints,
                'inf':        inf_list,
