...
| `sl_board_cdp.js` | ✅ | SpecLens 보드/변경점검 ↔ /sl-viewer 세션 CDP 다리(서버 없음) — `inject`(SR보드)/`drift`(변경점검)/`status`(진행)/`poll`(버튼클릭 큐 수집). playwright connectOverCDP(9222) | /sl-viewer STEP 3·5 |
| `detect_drift.py` | ✅ | 변경 감지(zero-LLM) — 스펙 anchors 소스 mtime > 스펙 mtime → STALE / 소스삭제 → MISSING → `docs/viewer/drift.json`. SpecLens 🔄 변경 점검 버튼이 트리거 | /sl-viewer STEP 5 |
...
| `scan_query_patterns.py` | ✅ | 실쿼리(SQL/XML)에서 **관찰 조인쌍(논리 FK) + 상시필터(soft-delete·테넌트, :BIND 포착)** 채굴 → `_machine/query_patterns.json`(영속 JIT 레이어). 레거시 FK미선언 시 유일한 JOIN 근거 | sl-recon STEP 5-0.5 / build_sch_static·AIDD |
| `build_sch_static.py` | ✅ | zero-token SCH 스켈레톤 — 컬럼(키 PK/FK열)·인덱스·FK+관찰조인(참조컬럼·출처)·상시필터 접이식·ERD·크로스링크·**frontmatter anchors(DDL/쿼리/라우터 근거소스)**·DB_{도메인}·DB_Schema, 의미는 `<!-- LLM-TODO -->`, sch_enrich_todo.json 산출 | sl-recon STEP 5-A |
...
| `scan_code_literals.py` | ✅ | 쿼리 코드값 리터럴 + JT_CODE 그룹신호(소스 정적) 추출 → 쿼리 의도복원 (4-2) → `_machine/code_literals.json` | sl-recon STEP 5-0.5 / ddd-db-agent |
