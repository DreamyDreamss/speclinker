...
| `build_si_graph.py` | ✅ | REQ↔FUNC↔코드 SI 트레이싱 그래프 빌드 | sl-recon-doc STEP 9-0-1 |
| `func_context_bundle.py` | ✅ | FUNC-ID별 스펙(INF/SCH/UIS) 컨텍스트 자동 수집 | sl-aidd STEP 2 |
| `req_scan.py` | ✅ | linked_func/linked_req 주석 스캔 → FUNC_MAP 커버리지 갱신 | sl-aidd STEP 5 / sl-dev 완료 후 |
...
| `link_inf_sch.py` | ✅ | INF → SCH 연결 패치 (`[TBD]` → `[[SCH-XXX]]`) | sl-recon-inf STEP 8-1 |
...
```
sl-recon 흐름:
  scan_source.js → resolve_call_chain.py → dispatch_inf_gen.py → [ddd-api-agent × N배치]
                → sch_draft/ → ddd-db-agent
...
sl-recon-inf 흐름:
  uis_api_hints.json + router_inventory_with_chain.json → [ddd-api-agent 정제]
  → link_inf_sch.py
