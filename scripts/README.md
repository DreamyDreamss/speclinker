...
|------|--------|------|------|------|------------|
| `resolve_call_chain.py` | ✅ | Controller→Service→DAO 호출 체인 추출 + Strategy 연동 | 워크스페이스 루트, profile.yaml | `_tmp/router_inventory_with_chain.json`, `_tmp/sch_draft/` | sl-recon STEP 4-2 |
| `screen_inventory.py` | ✅ | BFS 캡처 결과에 소스 파일 경로 역매핑 보강 | `.understand-anything/knowledge-graph.json` | `_tmp/screen_inventory.json` | sl-recon-uis STEP 6-2-3-B fallback |
| `build_funcs_index.py` | ✅ | spec.md + INF → FUNC_MAP 통합 인덱스 생성 | `docs/05_설계서/`, INF 파일 | `_tmp/funcs_index.json` | sl-recon STEP 6 |
...
| `link_uis_inf.py` | ✅ | UIS spec.md §5의 URL → INF 링크 패치 (LLM 없음) | UIS spec.md, INF 파일 | spec.md §5 인라인 수정 | sl-recon-uis STEP 6-4 |
| `link_inf_sch.py` | ✅ | INF → SCH 연결 패치 (`[TBD]` → `[[SCH-XXX]]`) | INF 파일, SCH 파일 | INF 파일 `linked_sch` 필드 | sl-recon-inf STEP 8-1 |
...
|------|--------|------|------|------|----------|
| `scan_source.js` | ✅ | 제로-LLM 정적 소스 스캔 → source_index.json (form/api kind 포함) | 워크스페이스 루트 | `_tmp/source_index.json` | sl-recon STEP 1 |
| `ai_nav.js` | ✅ | Chrome CDP BFS 탐색 (snapshot/click/capture) | Chrome --remote-debugging-port | BFS 결과 + 캡처 | sl-recon-uis STEP 6-2 |
| `detect_capture_strategy.js` | ✅ | 앱 유형 기반 캡처 전략 탐지 | Chrome CDP | `_tmp/capture_config.json` | sl-recon-uis 실행 전 |
...
```
sl-recon 흐름:
  scan_source.js → resolve_call_chain.py → ddd-api-agent → ddd-db-agent
                → screen_inventory_static.json (form routes)
                → link_inf_sch.py → ia_map_builder.py

sl-recon-uis 흐름:
  detect_capture_strategy.js → ai_nav.js (BFS)
  → [capture.js → annotate_preview.py → generate_uis_spec.py]
  → link_uis_inf.py → build_funcs_index.py
