...
| `gen_docsify.py` | ✅ | 산출물 스캔 → SpecLens `spec_index.json` 생성 + 뷰어 자산 복사. **관계 필드 보강**: `uis.inf_ids`(apis→INF 해소), `inf.sch_ids`(INF→SCH 역인덱스), `*.func`(FUNC_MAP 역매핑), `gaps`(연결 미연결 집계) — 모두 추가(하위호환, 데이터 없으면 빈값) | sl-viewer STEP 1 |
