...
| `gen_docsify.py` | ✅ | 산출물 스캔 → SpecLens `spec_index.json` 생성 + 뷰어 자산 복사. **관계 필드**: `uis.inf_ids`·`inf.sch_ids`·`*.func`·`gaps` + **`funcs[]`**(FUNC_MAP 파싱: id/name/domain/uis/inf/sch)·`domains[].overview` — 모두 추가(하위호환, 데이터 없으면 빈값) | sl-viewer STEP 1 |
